...
function get(url) {
	request.get(url, function (error, response, body) {
	    ...
	    k = k+1;
	    if (k == N) {
	    		fs.writeFileSync("INTERMAGNET_"+cadence+"_full.json",JSON.stringify(cats));
	    		callback();
	    	}	    	
	});
}
...
function callback() {
	var data = fs.readFileSync("INTERMAGNET_"+cadence+"_full.json");
	eval("cats =" + data);
	var urls = [];
	...
	fs.writeFileSync("INTERMAGNET_"+cadence+".json","var list = " + JSON.stringify(list));
